...
const audioMoeda = new Audio('/jogo-mario-master/audio/moeda.mp3');

const audioErrou = new Audio('/jogo-mario-master/audio/errou.mp3');
...
if(window.SpeechRecognition || window.webkitSpeechRecognition){
    ...
    gravador.onresult = function(event){
        transcicaoAudio = event.results[0][0].transcript.toUpperCase();
        respostaCorreta = document.getElementById('cor-na-caixa').innerText.toUpperCase();
        
        if(transcicaoAudio === respostaCorreta){ 
            atualizaPontuacao(1);
        }else{
            atualizaPontuacao(-1);
        }
